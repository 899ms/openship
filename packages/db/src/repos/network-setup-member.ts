  ...
  return db.transaction(async (tx) => {
    ...
    if (replacementId) {
      ...
      if (!preparation || preparation.inputHash !== inputHash)
        throw conflict("This request already removes a different server. Open the updated setup.");
      return {
        preparation,
    ...
    if (
      ...
      throw conflict("Setup is still running. Wait for it to finish before removing a server.");
    if (
      sourcePreparation &&
    ...
    if (sequence !== target.sequence)
      throw conflict("Setup changed. Reload its saved progress before removing a server.");
