...
state = {'exists': False, 'up': False, 'mtu': 1400, 'addresses': [], 'routes': [], 'peers': [], 'allowedIps': {}, 'publicKey': '', 'listenPort': None, 'enabled': False, 'firewall': False}
commands = []
def command(args, **kwargs):
 ...
 if args[:2] == ['wg', 'pubkey']: output = 'public:' + kwargs['input'].strip()
  ...
  if args == ['ip', '-d', '-j', 'addr', 'show']:
   ...
   output = json.dumps([{'dst': value.split('/')[0], 'dev': 'oswg' + 'a' * 10, 'prefsrc': state['addresses'][0].split('/')[0]} for value in state['routes']])
...
describe.skipIf(!pythonAvailable)("WireGuard transport gate and recovery", () => {
  ...
  it.each([false, true])("accepts a missing receipt only when the original network is intact (existing: %s)", async (existing) => {
  ...
  ] as const)("refuses to acknowledge missing recovery state while the live network differs: %s", async (scenario, existing) => {
    const { final } = await run(scenario, existing);
    expect(final.result.error).toContain("no longer matches this operation or its original configuration");
    expect(final.receipt).toBeNull();
    expect(final.commands).not.toContainEqual(["sh", "-c", "cancel-owned-timer"]);
