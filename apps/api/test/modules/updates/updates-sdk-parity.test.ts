...
vi.mock("@repo/platform/engine/modules/projects/project-crud.service", async original => ({
  ...await original<object>(), resolveUpstreamDrift: provider.poll,
}));
vi.mock("@repo/platform/engine/modules/deployments/build.service", async original => ({
  ...await original<object>(), redeployBuildSession: provider.redeploy,
}));

installFakeRunner();
const app = new Hono().onError(handleApiError).route("/api/health", healthRoutes).route("/api/updates", updatesRoutes);
...
beforeEach(() => {
  provider.poll.mockReset().mockImplementation(async (_ctx, project) => ({
    supported: true, mode: "commit", key: commitSourceKey(project), latestSha: latest, latestMessage: "Next version",
  }));
...
async function clients(owner: SeededOwner) {
  ...
  const ship = createShip({ platform: getPlatformKernel(), identity: { resolve: async () => ({ user: { id: user.id, email: user.email, name: user.name }, sessionId: "updates-test" }) } });
  return {
...
async function project(owner: SeededOwner, name: string, deployed = true) {
  const input = { organizationId: owner.orgId, name, slug: `${owner.userId}-${name}`, gitProvider: "github", gitOwner: "example", gitRepo: name, gitBranch: "main" };
  const group = await repos.projectGroup.create(input);
  const row = await repos.project.create({ ...input, groupId: group.id });
  if (deployed) {
    const deployment = await repos.deployment.create({ organizationId: owner.orgId, projectId: row.id, branch: "main", status: "ready", commitSha: shipped });
...
describe("updates shared SDK/HTTP operations", () => {
  it("shares the upstream cache, live drift and result presentation", async () => {
    const owner = await seedOwner(), p = await project(owner, "release"), c = await clients(owner);
    ...
    expect(items).toMatchObject([{ projectId: p.id, kind: "commit", behind: true, currentLabel: shipped.slice(0, 7), latestLabel: latest.slice(0, 7), checkedAt: expect.any(String), detail: { branch: "main" } }]);
    expect(provider.poll).toHaveBeenCalledTimes(1);
    await db.update(schema.deployment).set({ commitSha: latest }).where(eq(schema.deployment.id, p.activeDeploymentId!));
    for (const client of [c.native.updates, c.remote.updates]) expect(await client.list({ behindOnly: true })).toEqual([]);
  });
  ...
  it("filters inaccessible projects before polls and counts, including create-only grants", async () => {
    ...
    for (const [resourceType, resourceId, permissions] of [["updates", "*", ["read", "write"]], ["project", "*", ["create"]], ["project", visible.id, ["read"]]] as const)
    ...
    for (const client of [c.native.updates, c.remote.updates]) {
      ...
      for (const id of [visible.id, hidden.id, foreign.id]) await expect(client.apply(id)).rejects.toMatchObject({ code: "NOT_FOUND" });
    }
  ...
  it("retains the update trigger and records one audit through either interface", async () => {
    const owner = await seedOwner(), p = await project(owner, "apply"), c = await clients(owner);
    provider.redeploy.mockResolvedValue({ project_id: p.id, deployment_id: "new-deployment" });
    for (const client of [c.native.updates, c.remote.updates])
      expect(await client.apply(p.id)).toEqual({ project_id: p.id, deployment_id: "new-deployment" });
  ...
  it("refuses undeployed updates, invalid inputs and revoked membership before provider work", async () => {
    const owner = await seedOwner(), p = await project(owner, "undeployed", false), c = await clients(owner);
    for (const client of [c.native.updates, c.remote.updates]) {
      await expect(client.apply(p.id)).rejects.toMatchObject({ code: "VALIDATION_ERROR" });
      await expect(client.list({ behindOnly: "yes" } as never)).rejects.toMatchObject({ code: "VALIDATION_ERROR" });
    }
    await db.delete(schema.member).where(eq(schema.member.userId, owner.userId));
    for (const client of [c.native.updates, c.remote.updates]) await expect(client.scan()).rejects.toBeDefined();
