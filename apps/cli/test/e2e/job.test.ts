...
describe("openship job", () => {
  ...
  it("replaces a legacy file target when --server is supplied", async () => {
    ...
    fetchStub = stubFetch(() => ({ json: { data: job } }));
    expect((await runCommand(command, ["create", "--file", file, "--server", "new-server"])).code).toBe(0);
    expect(fetchStub.calls[0].body).toEqual({ label: "sync", command: "true", serverIds: ["new-server"] });
  });
  it.each(["null", "[]", '"private-config-value"'])("rejects non-object configuration %s before requesting", async (contents) => {
    const file = join(dir, "job.json");
    writeFileSync(file, contents);
    fetchStub = stubFetch(() => ({ json: { data: job } }));
    const result = await runCommand(command, ["update", "custom:1", "--file", file]);
    expect(result.code).toBe(1);
    expect(result.err).toContain("Job configuration must be a JSON object");
    expect(result.out + result.err).not.toContain("private-config-value");
    expect(fetchStub.calls).toHaveLength(0);
  });
  it("does not echo invalid configuration contents in parse errors", async () => {
    const file = join(dir, "job.json");
    writeFileSync(file, "private-config-value");
    fetchStub = stubFetch(() => ({ json: {} }));
    const result = await runCommand(command, ["create", "--file", file]);
    expect(result.code).toBe(1);
    expect(result.err).toContain("Job configuration must be valid JSON");
    expect(result.out + result.err).not.toContain("private-config-value");
    expect(fetchStub.calls).toHaveLength(0);
  });
  it.each([
