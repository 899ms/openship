...
describe("openship job", () => {
  it.each([false, true])("lists authenticated jobs with JSON mode %s", async (json) => {
    ...
    const rows = [{ key: "custom:1", label: "sync", actionConfig: { command: "true" } }];
  ...
  it("loads advanced settings from a file with flag precedence", async () => {
    ...
    const config = { label: "old", serverIds: ["s1", "s2"], env: { MODE: "ci" }, secrets: { TOKEN: "private" }, retry: { maxAttempts: 2, backoffSeconds: 5 } };
    writeFileSync(file, JSON.stringify(config));
    fetchStub = stubFetch(() => ({ json: { data: { key: "custom:1" } } }));
  ...
  it("lists history with a limit", async () => {
    fetchStub = stubFetch(() => ({ json: { data: [{ id: "r1", status: "success" }] } }));
  ...
  it.each([false, true])("reads stored logs with JSON mode %s", async (json) => {
    (await import("../../src/lib/output")).setJsonMode(json);
    const run = { id: "r/1", output: "saved output", error: null, status: "success" };
