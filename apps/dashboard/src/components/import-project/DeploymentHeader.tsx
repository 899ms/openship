...
export function DeploymentHeader({ onRedeploy, serviceCount, decisionPending = false }: {
  ...
  const StatusIcon = decisionPending || hasWarning ? "warning"
    : working ? "spinner" : deploymentStatus === "ready" ? "check-circle" : "x-circle";
  ...
  return (
    <header className="mb-6 space-y-4">
      <nav aria-label={t.dashboard.nav.deployments} className="flex min-w-0 items-center gap-2 text-sm text-muted-foreground">
        ...
        <UiIcon name="chevron-right" className="size-3.5 shrink-0 rtl:rotate-180" aria-hidden />
        ...
        <div className="min-w-0 flex-1 basis-72">
          <div className="flex flex-wrap items-center gap-3">
            ...
            <span className={`inline-flex items-center gap-1.5 rounded-full px-2.5 py-1 text-xs font-medium ${tone}`}>
              <UiIcon name={StatusIcon} className={`size-3.5 shrink-0 ${working ? "animate-spin" : ""}`} aria-hidden />
              {status}
            </span>
          </div>
          {(source || config.branch || serviceCount) ? (
            <div className="mt-2 flex flex-wrap items-center gap-x-4 gap-y-1 text-sm text-muted-foreground">
              ...
              {config.branch && <span className="inline-flex min-w-0 items-center gap-1.5"><UiIcon name="git-branch" className="size-3.5 shrink-0" aria-hidden /><span className="break-all">{config.branch}</span></span>}
